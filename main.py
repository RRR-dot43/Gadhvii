...
from config import api_id, api_hash, bot_token, log_channel, force_sub_channel
...
@app.route('/')
def home():
    return "Bot is alive! & Made By @MrGadhvii "
...
BOT_SEMAPHORE = asyncio.Semaphore(5)
SESSION_POOL = {}
...
async def update_user_data(user: User, joined: bool = False):
    users_data = load_users()
    user_id = str(user.id)  # Convert to string for JSON compatibility
    
    if user_id not in users_data["users"]:
        users_data["total_users"] += 1
        users_data["users"][user_id] = {
            "name": user.first_name,
            "username": user.username if user.username else None,
            "joined_date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "joined_force_sub": joined
        }
    else:
        users_data["users"][user_id]["joined_force_sub"] = joined
        
    save_users(users_data)

# Force subscription check
async def force_sub_check(bot: Client, message: Message) -> bool:
    try:
        ...
        if user.status in [ChatMemberStatus.LEFT, ChatMemberStatus.BANNED]:
            ...
            await message.reply(
                text="**❌ Access Denied!\n\n🔐 To use this bot, you need to join our channel first.\n\n1️⃣ Click 'Join Channel' to join\n2️⃣ Click 'Verify' after joining**",
        ...
        await message.reply(
            text="**⚠️ Something went wrong!\n\nPlease try these steps:\n1️⃣ Leave the channel and join again\n2️⃣ Click 'Verify' after rejoining\n\nIf problem persists, contact support.**",
            reply_markup=InlineKeyboardMarkup(buttons),
            quote=True
        )
        return False

@bot.on_callback_query(filters.regex("^verify_sub$"))
async def verify_subscription(bot, callback_query):
    try:
        user = await bot.get_chat_member(force_sub_channel, callback_query.from_user.id)
        if user.status not in [ChatMemberStatus.LEFT, ChatMemberStatus.BANNED]:
            # Update user data with subscription status
            await update_user_data(callback_query.from_user, joined=True)
            ...
            await callback_query.message.edit_text(
                "**✅ Verification Successful!\n\nYou can now use the bot. Please select an option below:**",
        ...
        await callback_query.answer("⚠️ Error occurred! Please leave channel, rejoin and try again.", show_alert=True)

# Log channel function
async def send_to_log_channel(bot: Client, user: User, file_path: str, caption: str):
    try:
...
async def clean_terminal():
    while True:
        try:
            ...
            logging.error(f"Error in terminal cleaner: {e}")
            await asyncio.sleep(60)

# Session pool manager
async def get_session(user_id: int) -> aiohttp.ClientSession:
    async with SESSION_LOCK:
        if user_id not in SESSION_POOL:
            connector = aiohttp.TCPConnector(limit=MAX_CONNECTIONS)
            SESSION_POOL[user_id] = aiohttp.ClientSession(connector=connector)
        return SESSION_POOL[user_id]

async def cleanup_session(user_id: int):
    async with SESSION_LOCK:
        if user_id in SESSION_POOL:
            await SESSION_POOL[user_id].close()
            del SESSION_POOL[user_id]

# Helper function for handling long messages
async def safe_edit_message(message, text, max_length=4096):
    """Safely edit message text, handling message too long errors by splitting or sending as a file."""
    try:
        if len(text) <= max_length:
            return await message.edit(text)
        else:
            # Text is too long, send as a file instead
            with open("long_message.txt", "w", encoding="utf-8") as f:
                f.write(text)
            
            with open("long_message.txt", "rb") as f:
                await message.delete()
                result = await message.reply_document(
                    document=f,
                    caption="**The message was too long, so it has been sent as a file.**"
                )
            os.remove("long_message.txt")
            return result
    except Exception as e:
        logging.error(f"Error editing message: {e}")
        # Try to send a simpler message
        try:
            return await message.edit("**Message too long. Check the next file for details.**")
        except:
            logging.error("Failed to send error message")
            return None

# Function for handling HTTP requests with retries and backoff
async def process_request_with_retries(session, url, method="GET", headers=None, params=None, json_data=None, retries=3, backoff_factor=2):
    """Wrapper for HTTP requests with retries and backoff"""
    async with REQUEST_SEMAPHORE:  # Limit concurrent requests
        for attempt in range(retries):
            try:
                ...
                logging.exception(f"Unexpected error in request: {e}")
                return None
...
# Optimize ThreadPool and connection limits for better stability
MAX_CONNECTIONS = 100
...
def handle_exception(loop, context):
    ...
    logging.info("Continuing bot execution despite error")

@bot.on_message(filters.command(["start"]))
async def start(bot, message):
    # Update user data
    await update_user_data(message.from_user)
    
    # Force subscription check
    if not await force_sub_check(bot, message):
        return
    ...
    await message.reply_photo(
        ...
        caption="**Developer - @CoreXExtractor\nPLEASE👇PRESS👇HERE**",
        quote=True,
        reply_markup=reply_markup
    )
...
async def fetch_pwwp_data(session: aiohttp.ClientSession, url: str, headers: Dict = None, params: Dict = None, data: Dict = None, method: str = 'GET') -> Any:
    # Check if session is actually an aiohttp.ClientSession
    if not isinstance(session, aiohttp.ClientSession):
        logging.error(f"Invalid session object type: {type(session)}")
        return None
        
    return await process_request_with_retries(
        session=session,
        url=url,
        method=method,
        headers=headers,
...
async def pwwp_callback(bot, callback_query):
    if not await force_sub_check(bot, callback_query.message):
        return
...
async def process_pwwp(bot: Client, m: Message, user_id: int):
    ...
    CONNECTOR = aiohttp.TCPConnector(limit=MAX_CONNECTIONS, loop=loop)
                ...
                for file in files:
                    ...
                    try:
                        with open(file, 'rb') as f:
                            ...
                            # Send to log channel
                            await send_to_log_channel(bot, m.from_user, file, caption)
...
async def fetch_appx_html_to_json(session, url, headers=None, data=None):
    try:
        # Check if session is actually an aiohttp.ClientSession
        if not isinstance(session, aiohttp.ClientSession):
            logging.error(f"Invalid session object type in fetch_appx_html_to_json: {type(session)}")
            return None
...
async def process_appxwp(bot: Client, m: Message, user_id: int):
    async with BOT_SEMAPHORE:  # Limit concurrent processing
        loop = asyncio.get_event_loop()
        CONNECTOR = aiohttp.TCPConnector(limit=MAX_CONNECTIONS, loop=loop)
            ...
            try:
                ...
                if not (api.startswith("http://") or api.startswith("https://")):
                    ...
                    if matches:
                        ...
                        await safe_edit_message(editable, f"**Send index number of the Batch to download.\n\n{text}**")
                ...
                if courses:
                    if total > 50:
                        ...
                        for cnt, course in enumerate(courses):
                            ...
                            price = course["price"]
                            text += f'{cnt + 1}. ```\n{name} 💵₹{price}```\n'
                        await safe_edit_message(editable, f"**Send index number of the course to download.\n\n{text}**")
                else:
                    ...
                    with open(f"{clean_file_name}.txt", 'w', encoding="utf-8") as f:
                        for output_line in all_outputs:
                            try:
                                ...
                                safe_line = ''.join(c if ord(c) < 128 else '?' for c in output_line)
                                f.write(safe_line)
                                logging.warning(f"Fixed Unicode encode error in line: {safe_line[:30]}...")
...
if __name__ == "__main__":
    ...
    try:
        # Start terminal cleaner and self-ping service
        loop.create_task(clean_terminal())
        ...
        async def cleanup():
            async with SESSION_LOCK:
                for user_id in list(SESSION_POOL.keys()):
